...
class Calibration extends React.Component<any, any>{
  ...
  docLoad() {
    ...
    this.clearCanvas();
    document.getElementById("calibration_done")?.style.setProperty('display', 'none');
    document.getElementById('Pt5')?.style.setProperty('display', 'none');
    document.getElementById("calibration_in_progress")?.style.setProperty('display', 'block');
  ...
  async calcPointClick(node: any) {
    ...
    this.setState((prevState: any) => ({
      ...
      if (this.pointCalibrate === 8){
          var cal_in_progress = document.getElementById("calibration_in_progress") as HTMLElement;
      ...
      if (this.pointCalibrate >= 9){ // last point is calibrated
          ...
          var cal_done = document.getElementById("calibration_done") as HTMLElement;
  ...
  render() {
    return (
      <div id="container">   
        <canvas ref={this.canvasRef} id="plotting_canvas" width={500} height={500} style={{ cursor: 'crosshair' }} />
        ...
        <div className="calibrationDiv">
          {Object.keys(this.state.clickCounts).map((pointId) => (
            <button
              ...
              id={pointId}
              onClick={() => this.calcPointClick(document.getElementById(pointId))}
              disabled={this.state.clickCounts[pointId] >= 5}
              style={{
                opacity: (0.2 * (this.state.clickCounts[pointId] + 1)).toString(),
                backgroundColor: this.state.clickCounts[pointId] >= 5  ? '#E2B714' : '#D1D0C5'
              }}
            />
          ))}
        </div>
        ...
        <button type="button" id='restart_calibration' className="top-button" onClick={this.handleRestart}>Recalibrate</button>
      </div>
    );
  }
}
